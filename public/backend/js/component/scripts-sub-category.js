    $('body').on('click', '.status-c', function(){
	   ...
	   $.get('/internal-bkn/change-status-sub-category?cid='+ category_id +'-'+ status_id, function(result){
		   if(result == 'success'){
			   if(status_id == 0){
	...
	$('body').on('click', '#confirm-ok', function(){
		...
		$.get('/internal-bkn/delete-sub-category?did='+ did, function(deleted){
			if(deleted == 'success'){
	...
	$('body').on('click', 'ul.pagination a', function(ev){
		...
		var first_li   = '<li><a href="/internal-bkn/loading-sub-category-list?page=1">«</a></li>';
		var last_li    = '<li><a href="/internal-bkn/loading-sub-category-list?page='+ total_li +'" rel="next">»</a></li>';
			...
			for(var k=1; k<=total_li; k++){
				if($.isNumeric(active_val) == true){
					...
					if(parseInt(active_val) == k){
						...
						pagin_li += '<li><a href="/internal-bkn/loading-sub-category-list?page='+ k +'">'+ k +'</a></li>';
					}
				}else{
					pagin_li += '<li><a href="/internal-bkn/loading-sub-category-list?page='+ k +'">'+ k +'</a></li>';
				}
		...
		$.get('/internal-bkn/loading-sub-category-list?page='+ page, function(data){
			if(data != ""){
	...
	$('#find_sub_category').on('submit', function(e){
		e.preventDefault();
		...
		if(pagin != ''){
			...
			if(total_li > 0){
				for(var k=1; k<=total_li; k++){
					if(k == 1){
						pagin_li += '<li class="active"><a href="/internal-bkn/loading-sub_category-list?page='+ k +'">'+ k +'</a></li>';
					}else{
						pagin_li += '<li><a href="/internal-bkn/loading-sub_category-list?page='+ k +'">'+ k +'</a></li>';
					}
				}
			} // End total_li
