    $('body').on('click', '.status-a', function(){
       var val_id = $(this).attr('id');
	   var status_id = val_id.split('-')[2];
	   var article_id= val_id.split('-')[1];
	   
	   $.get('/internal-bkn/change-status-article?aid='+ article_id +'-'+ status_id, function(result){
		   if(result == 'success'){
			   if(status_id == 0){
				   $('#'+ val_id).find('i').attr('class', 'no-button');
				   $('#'+ val_id).attr('id', 'status-'+ article_id +'-1');
			   }else{
				   $('#'+ val_id).find('i').attr('class', 'yes-button');
				   $('#'+ val_id).attr('id', 'status-'+ article_id +'-0');
			   }
		   }
	   });
    });
	...
	$('body').on('click', '.del-button', function(){
		...
		var del_title = $(this).parent().parent().find('.article-title').text();
	...
	$('body').on('click', '#confirm-ok', function(){
		...
		$.get('/internal-bkn/delete-article?did='+ did, function(deleted){
			if(deleted == 'success'){
	...
	$('body').on('click', 'ul.pagination a', function(ev){
		...
		var first_li   = '<li><a href="/internal-bkn/loading-article-list?page=1">«</a></li>';
		var last_li    = '<li><a href="/internal-bkn/loading-article-list?page='+ total_li +'" rel="next">»</a></li>';
			...
			for(var k=1; k<=total_li; k++){
				if($.isNumeric(active_val) == true){
					...
					if(parseInt(active_val) == k){
						...
						pagin_li += '<li><a href="/internal-bkn/loading-article-list?page='+ k +'">'+ k +'</a></li>';
					}
				}else{
					pagin_li += '<li><a href="/internal-bkn/loading-article-list?page='+ k +'">'+ k +'</a></li>';
				}
		...
		$.get('/internal-bkn/loading-article-list?page='+ page, function(data){
			if(data != ""){
	...
	$('#find_article').on('submit', function(e){
		e.preventDefault();
		...
		if(pagin != ''){
			...
			if(total_li > 0){
				for(var k=1; k<=total_li; k++){
					if(k == 1){
						pagin_li += '<li class="active"><a href="/internal-bkn/loading-article-list?page='+ k +'">'+ k +'</a></li>';
					}else{
						pagin_li += '<li><a href="/internal-bkn/loading-article-list?page='+ k +'">'+ k +'</a></li>';
					}
				}
